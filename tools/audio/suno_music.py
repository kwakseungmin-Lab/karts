"""Suno AI music generation — DEPRECATED (Suno API removed).

This tool previously called the Suno AI API via sunoapi.org (paid, requires SUNO_API_KEY).
All calls are now routed to FreesoundMusic (free with API key) which sources
Creative Commons music from Freesound.org.

Reason for replacement: Suno API requires a paid subscription and sends prompts
to Suno cloud. Freesound provides free CC-licensed music via a free API key with
no ongoing cost.
"""
# REPLACED: Suno API removed — routing to freesound_music (free open alternative)
...
class SunoMusic(BaseTool):
    ...
    def execute(self, inputs: dict[str, Any]) -> ToolResult:
        ...
        return FreesoundMusic().execute(inputs)
