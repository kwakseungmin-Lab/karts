"""Doubao Speech TTS — DEPRECATED (ByteDance Doubao API removed).

This tool previously called the Volcengine Doubao Speech API (paid, requires DOUBAO_SPEECH_API_KEY).
All calls are now routed to PiperTTS (open-source, runs locally, no API key needed).

Reason for replacement: Doubao Speech API requires a paid ByteDance/Volcengine API key
and sends text to ByteDance cloud. Piper TTS runs locally via a compiled binary with
no ongoing cost and no data leaving the machine.
"""
# REPLACED: ByteDance Doubao Speech API removed — routing to piper_tts (open-source)
