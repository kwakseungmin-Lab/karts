#!/usr/bin/env python3
"""Generate scene07_cut01 frame image."""
...
PROMPT = (
    ...
    "nodachi sword held cautiously, right thigh with shallow cut), "
    "heavy breathing visible, morning fog halfway cleared, soft lateral light on armor, "
...
OUT_RAW = PROJECT_ROOT / "final/frames/scene07_cut01.png"
OUT_HD = PROJECT_ROOT / "final/frames/scene07_cut01_hd.png"
...
def main() -> None:
    ...
    # Build ref list: character refs first, then background
    ref_list = list(available_refs)
    if bg_available:
        ref_list.append(BACKGROUND_REF)

    if ref_list:
        print(f"Using edit endpoint with {len(ref_list)} ref(s): {[str(p) for p in ref_list]}")
